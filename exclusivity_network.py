...
import warnings
import os
from utils import get_edge_weight, cancers, subtypes

warnings.filterwarnings('ignore', category=RuntimeWarning)


for c, cancer in enumerate(cancers):
    ppi = pd.read_csv('./gene_sample_overlapped/'+cancer+'/ppi.csv',header=None,index_col=None)

    data_mut = list()
    data_methy = list()
    data_expr = list()
    for s, subtype in enumerate(subtypes[cancer]):
        data_mut.append(pd.read_csv('./gene_sample_overlapped/'+cancer+'/'+subtype+'/mut.csv',header=0,index_col=0))
        data_methy.append(pd.read_csv('./gene_sample_overlapped/'+cancer+'/'+subtype+'/0.05/methy_mut.csv',header=0,index_col=0))
        data_expr.append(pd.read_csv('./gene_sample_overlapped/'+cancer+'/'+subtype+'/0.05/expr_mut.csv',header=0,index_col=0))
    ...
    edge_exclusivity = np.zeros((len(ppi_idx),len(subtypes[cancer])))
    for s, subtype in enumerate(subtypes[cancer]):
        edge_exclusivity[:,s] = get_edge_weight(data_mut[s], ppi_idx) + get_edge_weight(data_methy[s], ppi_idx) + get_edge_weight(data_expr[s], ppi_idx)
    ...
    for s, subtype in enumerate(subtypes[cancer]):
        ...
        if len(exclusivity_network) > 0:
            ...
            if not os.path.exists('./exclusivity_network/'):
                os.makedirs('./exclusivity_network/')
            exclusivity_network.to_csv('./exclusivity_network/' + cancer + '_' + subtype + '.csv',index=None)
