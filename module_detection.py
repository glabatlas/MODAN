...
def generate_initial_modules(cancer, subtype):
    qualified_genes = get_NI_qualified_genes(cancer, subtype)
    exc_net = pd.read_csv('./me_network/' + cancer + '_' + subtype + '.csv',index_col=None,header=0)
    exc_net_genes = np.sort(np.union1d(exc_net.values[:,0],exc_net.values[:,1]))

    data_mut = pd.read_csv('./datasets/'+cancer+'/'+subtype+'/mut.csv',header=0,index_col=0)
    data_methy = pd.read_csv('./datasets_generate/'+cancer+'/'+subtype+'/methy_mut.csv',header=0,index_col=0)
    data_expr = pd.read_csv('./datasets_generate/'+cancer+'/'+subtype+'/expr_mut.csv',header=0,index_col=0)
...
def module_optimization(cancer, subtype):
    data_mut = pd.read_csv('./datasets/'+cancer+'/'+subtype+'/mut.csv',header=0,index_col=0)
    data_methy = pd.read_csv('./datasets_generate/'+cancer+'/'+subtype+'/methy_mut.csv',header=0,index_col=0)
    data_expr = pd.read_csv('./datasets_generate/'+cancer+'/'+subtype+'/expr_mut.csv',header=0,index_col=0)
    symbol_idx = pd.DataFrame(range(len(data_mut.index)),index=data_mut.index)
    ...
    ppi_weight_mut = get_edge_weight(data_mut.values, ppi_weighted)
    ppi_weight_methy = get_edge_weight(data_methy.values, ppi_weighted)
    ppi_weight_expr = get_edge_weight(data_expr.values, ppi_weighted)
    ppi_weighted = np.append(ppi_weighted, ((ppi_weight_mut + ppi_weight_methy + ppi_weight_expr)/3).reshape(-1,1), axis=1)
    res_table = pd.read_csv('./results/modules/'+cancer+'/'+subtype+'/initial_results.csv',header=None)
    module_list = []
    for i in range(len(res_table)):
        module = res_table.values[i,2::]
        module_genes = []
    ...
    while True:
        ...
        for i in range(len(module_list)):
            ...
            current_exclusivity = get_single_module_exclusivity(data_mut, data_methy, data_expr, current_module)
            if len(current_module) >= 3:
                ...
                for l in range(len(current_module)):
                    ...
                    new_weight[l] = get_single_module_exclusivity(data_mut, data_methy, data_expr, current_module_genes_idx_del)
    ...
    for i in range(len(module_list)):
        exclusivity_new[i] = get_single_module_exclusivity(data_mut, data_methy, data_expr, module_list[i])
    influence_new = get_all_module_influence(cancer, subtype, module_list)
    module_genename_list = []
    for i in range(len(module_list)):
        module_genename_list.append(data_mut.index[module_list[i]].to_list())
    ...
    clustering_results.columns = [f'column_{i}' for i in range(clustering_results.shape[1])]
    clustering_results = clustering_results.sort_values(by=['column_0','column_1'],ascending=False)
    clustering_results.to_csv('./results/modules/' + cancer + '/' + subtype + '/final_results.csv',header=None,index=None)