...
def feature_learning(cancer, subtype):
    walk_length = 60

    ppi = pd.read_csv('./me_network/' + cancer + '_' + subtype + '.csv',index_col=None,header=0)
    ...
    if not os.path.exists('./models/'):
        os.makedirs('./models/')
    weighted_model.save('models/'+ cancer +'_' + subtype + '.model')
    ...
    np.savetxt('./results/features/'+cancer+'_'+subtype+'_features.csv', out, delimiter=',')
